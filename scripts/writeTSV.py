...
def handleArticle(article, outFile):
    ...
    tsvFile.write(article.get("id") + "\t" + \
                ...
                str(len(urlFile[article.get("id")])) + "\n")
...
def main(inputDataset, outputFile):
    ...
    writeFile = open("/scratch/anamikas/hyp/v2/tsvFileStats.txt", "a")
    writeFile.write("The time taken to run the script for article words counting and tf-extractor " + str(totaltime))
    writeFile.close()

if __name__ == '__main__':
    tsvFile = open('/scratch/anamikas/hyp/v2/tsvFile.tab', 'a')
    trainFile = pickle.load(open('/scratch/anamikas/hyp/v2/outputs/train/byPublisher/articletrainbyPublisher.p', 'rb'))
    truthFile = pickle.load(open('/scratch/anamikas/hyp/v2/dataset/groundTruth/train/byPublisher/groundTruthbyPublisher.p', 'rb'))
    urlFile = pickle.load(open('/scratch/anamikas/hyp/v2/outputs/train/byPublisher/urlTypeWithoutEmptyStringsbyPublisherURL.p', 'rb'))
    trainFile['1371627'] = {}
    trainFile['1371627']['lengthwords'] = 0
    urlFile['1371627'] = ""
    main(*parse_options())
