...
def main(inputDataset, outputFile, typ):
    ...
    name = 'article' + typ
    with open('/scratch/anamikas/hyp/results/artTraining.json', 'w') as fp:
        json.dump(outDict, fp)
    pickle.dump(outDict, open(os.path.join('/scratch/anamikas/hyp/results/', name +'.p'), 'wb'))
    end = time.time()
    totaltime = end-start

    writeFile = open("/home/anamikas/work/Hyperpartisan-News-Detection/results/articleStatstrain.txt", "w")
