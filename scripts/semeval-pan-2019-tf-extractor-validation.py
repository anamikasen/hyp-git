...
def main(inputDataset, outputFile, typ):
    ...
    name = 'article' + typ
    pickle.dump(outDict, open(os.path.join('/scratch/anamikas/hyp/results/', name +'.p'), 'wb'))
    end = time.time()
    totaltime = end-start

    writeFile = open("/home/anamikas/work/Hyperpartisan-News-Detection/results/articleStatsvalidation.txt", "w")
    writeFile.write("The time taken to run the script for article words counting and tf-extractor validation" + name + str(totaltime))
    writeFile.close()
