...
def handleArticle(article, outFile):
    ...
    for a in html.find_all('a'):
        try:
            if a['href']!="":
                urls.append(a['href'])
                typeUrls.append(a['type'])
        except KeyError:
            continue
    # import pdb; pdb.set_trace()
    # urls = [a['href'] for a in html.find_all('a') if a['href']!=""]
    # typeUrls = [a['type'] for a in html.find_all('a') if a['href']!=""]
...
def main(inputDataset, outputFile, typ):
    ...
    name = 'urlTypeWithoutEmptyStrings' + typ
    pickle.dump(urlKeeper, open(os.path.join('/scratch/anamikas/hyp/v2/outputs/train/byPublisher/', name +'.p'), 'wb'))
    end = time.time()
    totaltime = end-start

    writeFile = open("/scratch/anamikas/hyp/v2/outputs/train/byPublisher/articleURLstats.txt", "w")
    writeFile.write("The time taken to get the URLs and type without empty strings " + name + str(totaltime))
    writeFile.close()
