...
def handleArticle(article, outFile):
    title[article.get("id")] = article.get("title")
...
def main(inputDataset, outputFile, typ):
    ...
    name = 'title' + typ
    pickle.dump(title, open(os.path.join('/scratch/anamikas/hyp/v2/outputs/validation', name +'.p'), 'wb'))
    end = time.time()
    totaltime = end-start

    writeFile = open("/scratch/anamikas/hyp/v2/outputs/validation/Titlestats.txt", "w")
    writeFile.write("The time taken to get the title is " + name + str(totaltime))
    writeFile.close()
